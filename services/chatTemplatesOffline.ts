...
export async function updateChatTemplateOffline(
  ...
  try {
    ...
    if (!navigator.onLine) {
      console.log('[ChatTemplatesOffline] Offline, queueing update');
      // Queue update profile dengan chat templates baru
      await syncManager.queueUpdate('profiles', {
        id: userProfile.id,
...
export async function addChatTemplateOffline(
  ...
  try {
    ...
    if (!navigator.onLine) {
      console.log('[ChatTemplatesOffline] Offline, queueing add');
      await syncManager.queueUpdate('profiles', {
        id: userProfile.id,
        chatTemplates: updatedTemplates,
      });
...
export async function deleteChatTemplateOffline(
  ...
  try {
    ...
    if (!navigator.onLine) {
      console.log('[ChatTemplatesOffline] Offline, queueing delete');
      await syncManager.queueUpdate('profiles', {
        id: userProfile.id,
        chatTemplates: updatedTemplates,
      });
...
export async function resetToDefaultTemplatesOffline(
  ...
  try {
    ...
    if (!navigator.onLine) {
      console.log('[ChatTemplatesOffline] Offline, queueing reset');
      await syncManager.queueUpdate('profiles', {
        id: userProfile.id,
