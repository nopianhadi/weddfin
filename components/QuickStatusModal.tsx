...
import { CheckCircleIcon, BellIcon } from '../constants';
...
export const QuickStatusModal: React.FC<QuickStatusModalProps> = ({
    ...
    const handleSave = async () => {
        if (!selectedStatus || selectedStatus === project.status) {
            showNotification('Pilih status yang berbeda');
            return;
        }
        
        setIsProcessing(true);
        try {
            await onStatusChange(project.id, selectedStatus, notifyClient);
            ...
            footer={
                <div className="flex items-center justify-end gap-2 md:gap-3 w-full">
                    ...
                    <button
                        type="button"
                        onClick={handleSave}
                        disabled={isProcessing || selectedStatus === project.status}
                    ...
                    >
                        {isProcessing ? (
                            ...
                            <>
                                ...
                                <span className="hidden sm:inline">Simpan & {notifyClient ? 'Notif Klien' : 'Tanpa Notif'}</span>
                                <span className="sm:hidden">Simpan</span>
                            </>
                        )}
                    </button>
                    ...
                    <div className="space-y-2 max-h-[35vh] md:max-h-80 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-brand-border scrollbar-track-transparent">
                        {statusConfig.map((status) => {
                            ...
                            return (
                                <label
                                    key={status.id}
                                    className={`flex items-center gap-2 md:gap-3 p-2.5 md:p-3 rounded-lg border-2 cursor-pointer transition-all ${
                                        isSelected
                ...
                <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3 md:p-4">
                    <label className="flex items-start gap-2 md:gap-3 cursor-pointer">
                        <input
                            ...
                            className="w-4 h-4 md:w-5 md:h-5 text-brand-accent rounded focus:ring-brand-accent mt-0.5 flex-shrink-0"
                        />
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-brand-text-secondary">
                                Kirim notifikasi otomatis ke klien tentang perubahan status proyek
                            </p>
                        </div>
                    </label>
                </div>
