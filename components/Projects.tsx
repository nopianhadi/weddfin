...
import { Project, PaymentStatus, TeamMember, Client, Package, TeamProjectPayment, Transaction, TransactionType, AssignedTeamMember, Profile, Revision, RevisionStatus, NavigationAction, AddOn, PrintingItem, Card, ProjectStatusConfig, SubStatusConfig, CustomCost } from '../types';
...
interface ProjectFormProps {
    ...
    onPayPrintingItem: (projectId: string, printingItemId: string, sourceCardId: string) => void;
    cards: Card[];
...
const ProjectForm: React.FC<ProjectFormProps> = ({
    ...
    onPayPrintingItem, cards, showNotification
}) => {
            ...
            <form onSubmit={onSubmit} className="space-y-6 form-compact form-compact--ios-scale">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-4 max-h-[70vh] overflow-y-auto pr-2">
                    ...
                    <div className="space-y-6">
                        ...
                        <section>
                            <h4 className="text-base font-semibold text-gradient border-b border-brand-border pb-2 mb-4">Biaya Operasional</h4>
                            <div className="grid grid-cols-2 gap-4">
                         ...
                         <section>
                            <h4 className="text-base font-semibold text-gradient border-b border-brand-border pb-2 mb-4">Output Fisik (Cetak)</h4>
                                            ...
                                            {isPaid ? (
                                                ...
                                                <div className="flex items-center gap-2 w-full sm:w-auto">
                                                    ...
                                                    <button 
                                                        ...
                                                        className="button-primary !text-xs !px-3 !py-2.5"
                                                    >
                                                        Bayar
                                                    </button>
                                                </div>
                        ...
                        {projects.map(project => {
                            ...
                            return (
                                <tr key={project.id} className="hover:bg-brand-bg">
                                    ...
                                    <td className="px-4 py-3"><span className={`px-2 py-1 text-xs font-medium rounded-full flex items-center justify-center gap-1.5 ${getPaymentStatusClass(project.paymentStatus)}`}><CreditCardIcon className="w-3 h-3" />{project.paymentStatus}</span></td>
            ...
            <div className="pt-6 max-h-[65vh] overflow-y-auto pr-2">
                {detailTab === 'details' && (
                    <div className="space-y-6">
                        ...
                        <div>
                            <h4 className="font-semibold text-gradient mb-3">Outfut Pengantin</h4>
...
export const Projects: React.FC<ProjectsProps> = ({ projects, setProjects, clients, packages, teamMembers, teamProjectPayments, setTeamProjectPayments, transactions, setTransactions, initialAction, setInitialAction, profile, showNotification, cards, setCards }) => {
    ...
    const handlePayForPrintingItem = async (projectId: string, printingItemId: string, sourceCardId: string) => {
        ...
        try {
            ...
            showNotification(`Pembayaran untuk "${printingItem.customName || printingItem.type}" berhasil.`);
        } catch (err) {
