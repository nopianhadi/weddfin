...
import { Project, Client, Transaction, TransactionType, ViewType, TeamMember, Card, FinancialPocket, PocketType, Lead, LeadStatus, TeamProjectPayment, Package, ClientFeedback, Contract, ClientStatus, NavigationAction, User, ProjectStatusConfig } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ projects, clients, transactions, teamMembers, cards, pockets, handleNavigation, leads, teamProjectPayments, packages, clientFeedback, contracts, currentUser, projectStatusConfig }) => {
    ...
    return (
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
            ...
            <div className="col-span-1 xl:col-span-12 widget-animate" style={{ animationDelay: '0ms' }}>
                <AIInsightWidget
                    projects={projects}
                    leads={leads}
            ...
            <div className="col-span-1 xl:col-span-12 grid grid-cols-2 gap-3 md:gap-6">
                ...
                <div className="widget-animate transition-transform duration-200 hover:scale-105" style={{ animationDelay: '200ms' }}>
                    <StatCard 
                        ...
                        colorVariant="purple"
                        description={`Proyek aktif adalah proyek yang statusnya bukan "Selesai" atau "Dibatalkan".\n\nProyek aktif memerlukan perhatian dan tindak lanjut untuk memastikan penyelesaian tepat waktu.\n\nKlik untuk melihat detail semua proyek aktif Anda.`}
                ...
                <div className="widget-animate transition-transform duration-200 hover:scale-105" style={{ animationDelay: '300ms' }}>
                    <StatCard 
                        ...
                        subtitle="Klien dengan proyek aktif" 
                        colorVariant="green"
                        description={`Klien aktif adalah klien yang memiliki minimal satu proyek yang sedang berjalan.\n\nMempertahankan hubungan baik dengan klien aktif sangat penting untuk bisnis Anda.\n\nKlik untuk melihat daftar klien aktif dan proyek mereka.`}
            ...
            <div className="col-span-1 xl:col-span-3 widget-animate cursor-pointer transition-transform duration-200 hover:scale-105" style={{ animationDelay: '1400ms' }} onClick={() => setActiveModal('contracts')}>
                <StatCard icon={<FileTextIcon className="w-5 h-5 md:w-6 md:h-6" />} title="Total Kontrak" value={contracts.length.toString()} subtitle="Kontrak yang terdaftar" colorVariant="purple" />
            </div>
                    ...
                    {clients.filter(c => projects.some(p => p.clientId === c.id && p.status !== 'Selesai' && p.status !== 'Dibatalkan')).map(client => {
                        ...
                        return (
                            <div key={client.id} className="p-4 bg-brand-bg rounded-lg hover:bg-brand-input transition-colors cursor-pointer" onClick={() => { setActiveModal(null); handleNavigation(ViewType.CLIENTS); }}>
                                ...
                                <p className="text-sm text-brand-text-secondary mt-1">{clientProjects.length} proyek aktif</p>
